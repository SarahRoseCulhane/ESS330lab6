...
# Create the first map colored by aridity
map_aridity <- ggplot(data = camels, aes(x = gauge_lon, y = gauge_lat)) +
  ...
  ggthemes::theme_map() +
  labs(title = "Site Map Colored by Aridity",
       color = "Aridity Index",
       x = "Longitude",
       y = "Latitude")

# Create the second map colored by p_mean
map_p_mean <- ggplot(data = camels, aes(x = gauge_lon, y = gauge_lat)) +
  ...
  ggthemes::theme_map() +
  labs(title = "Site Map Colored by Precipitation Mean",
...
Looking at the workflow rank result, it looks like the neutral network model outperforming the rand_forest model and the linear_reg model but the xgboost regression model is under performing all other models in the R-squared metric. Going forward, I would use the neutral network model.
...
```{r}
formula <- logQmean ~ aridity + p_mean + elev_mean  # create a forumula
...
I think the rand_forest model would be best because it has the highest R-squared value, suggesting a strong model fit.
